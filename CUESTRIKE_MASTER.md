...
### 🔗 Call-Shot Show Trigger (2026-08-10, by Buffy/Freebuff — per implementation_plan_callshot_trigger.md)
- ✅ **ปิดวงจร call-shot ครบ**: เพิ่ม `MaybeShowCallShotUI()` ใน `ChinesePoolGameManager` — โชว์ panel เมื่อ `IsCallShotRequired()` + ตาผู้เล่นจริง (ไม่ใช่ AI) → `ChinesePoolUIManager.Instance?.ShowCallShot(false, groupInt)` (1=RED, 2=YELLOW, 0=generic)
