...
### 🧹 CS0618 Find-API Modernize (2026-08-10, by Buffy/Freebuff — per implementation_plan_cs0618_cleanup.md)
- ✅ **ล้างคำเตือน CS0618 ทั่วโปรเจกต์** — `FindObjectOfType<T>()` → `FindFirstObjectByType<T>()` (Unity 6 ที่เพิ่มตั้งแต่ 2023.1) และ `FindObjectsOfType<T>()` → `FindObjectsByType<T>(FindObjectsSortMode.None)` (เร็วขึ้น + ไม่มี sort cost)
- ✅ **Mapping 4 patterns** (ครอบคลุมทุก site): plain / `Object.` / `GameObject.` (drop prefix legacy ที่ลบใน 2023+) / `(true)` flag (Inactive=Include)
- ✅ **36 call sites** ใน **21 ไฟล์** (13 runtime + 8 editor) — compile gate 0 errors, behavior preservation ตาม Unity 6 ScriptReference
- ✅ **Editor tools ที่ critical** (e.g. `Editor/ChinesePoolUISetup.cs` 5 calls, `Editor/NoirMemorySelfTest.cs` 4 calls) — ทำงานเดิมเป๊ะ (Object.FindFirstObjectByType คืน instance แบบเดียวกัน)
- 📝 Plan: `implementation_plan_cs0618_cleanup.md` + migration script `tools/migrate_find_api.py` (token-based, balanced-paren safe)
